...
Function EGNA_UniversalLoader(PathName,FileName,FileType,NewWaveName)
	...
	elseif(cmpstr(FileType,"BS_Suitcase_Tiff")==0)
		...
		//SUBH start
		string enrgy, poln
		wave poln_list = root:Packages:NikaNISTRSoXS:channels:en_polarization_setpoint
		wave enrgy_list = root:Packages:NikaNISTRSoXS:channels:en_energy_setpoint
		poln = num2str(100 + poln_list[imnum])				//changing to BL11012 convention of 100- and 190-pol to be compatible with polarization calculation function
		enrgy = num2str(enrgy_list[imnum])
		userFileName = cleanupname(userfilename,0)+"_"+ detectortype[0] +"_"+ enrgy + "eV" +"_"+poln  //compatible with groupplot stitch
		
//		if (cmpstr(detectortype[0], "S") == 0)													//compatible with manual stitiching code
//			userFileName = "lq_" + cleanupname(userfilename,0)+"_"+ enrgy +"_" + poln
//		elseif (cmpstr(detectortype[0], "W") == 0)
//			userFileName = "hq_" + cleanupname(userfilename,0)+"_"+ enrgy +"_" + poln
//		endif
			...
			if(V_Flag==0)
				For(ii=0;ii<100;ii+=1)
					...
					if(strlen(templine)<1)
						...
						templine = EG_N2G_ChangePartsOfString(templine,"  ","")
						templine = EG_N2G_ChangePartsOfString(templine,"\r","")
						templine = EG_N2G_ChangePartsOfString(templine,":","=")
